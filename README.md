...
When a **burned video** is sent back, the bot also writes a short, social-post-style
**caption in the target language** — generated by the LLM from the transcript to capture
what the clip is about and its hook, not a re-transcription. It's a separate LLM call
from the translation, capped at Telegram's 1024-character limit, and falls back to a
plain `Subtitled · <language>` line if generation fails. (The `.srt` is sent with a
simple label instead.)
